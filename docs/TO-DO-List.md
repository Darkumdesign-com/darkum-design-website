...
- [x] Build a How It Works section, four-step Choose/Place/Change/Own stepper, right after About/Story. **Complete 2026-09-01/02**, including Arabic copy, rebuilt from an initial horizontal 4-column layout into a vertical icon/arrow/text stepper matching a specific reference image, heading added, border hugs the content column, down-arrow connectors rebuilt as independent icon blocks, orphaned hidden block deleted, pattern background applied and rotated 45°, connector arrows' screen-reader labels cleaned up, mobile margin fixed after three rounds (two genuinely different root causes found along the way), and Egyptian-colloquial Arabic added for the heading and all four steps. See `docs/handoff/homepage-and-header-footer.md`.
...
- [x] Build the About/Story section as its own separate section, dual-column bilingual grid, desktop CSS-Grid auto-placement, mobile collapses to a single column in correct paired order (same mobile-pairing fix as the coming-soon page). **Structure built 2026-08-26**, `dkd-about-row`. **Real copy applied and section rebuilt into native, non-technical-editor-friendly blocks 2026-09-06** (Dema confirmed a reviewer, not a blocker, real copy no longer waiting on her turnaround), including a new lang-attribute mechanism for the native blocks and a height fix for the image column's new 3-image slider. See `docs/handoff/homepage-and-header-footer.md`.
- [x] Add floating WhatsApp button (`wa.me/201037888900`)
- [ ] **Homepage written content still outstanding** (materials/craftsmanship copy, or any other unfinished marketing copy). **No longer blocked on Dema as of 2026-09-06**, moved here from Phase 2, Bido confirmed by phone she's a reviewer, not a gatekeeper ("Do your best, and Dema will edit it if needed"), About/Story's copy was drafted and applied directly under this clarification, same approach applies to whatever's still outstanding here
- [ ] **Decide whether to restructure How It Works/Testimonials/Contact's bilingual copy for proper `lang` attribute accessibility.** Those sections mix English and Arabic in single blocks (unlike Our Story/Hero, now fixed), tagging a mixed block with one `lang` value would be wrong either way; fixing it properly means splitting each line into separate blocks first, a content-restructuring decision, not a quick pass. See `docs/handoff/homepage-and-header-footer.md`.
