...
- [ ] Build a Testimonials section as its own separate section, placeholder structure, same "build now with obviously-fake bracketed placeholder, swap content in later, no rebuild needed" pattern as About/Story. Not blocked on Dema the way other homepage copy is, real testimonials require actual customer reviews post-launch regardless of who owns copywriting, a separate and likely later dependency than Dema's turnaround. Layout probably cards or a carousel rather than the dual-column bilingual grid, testimonials read differently than prose paragraphs, worth its own layout conversation when ready to build.
- [-] Build the About/Story section as its own separate section (not merged with materials/craftsmanship or anything else). Dual-column responsive grid: desktop shows EN/AR side by side via CSS Grid auto-placement (source order EN paragraph, AR paragraph, EN paragraph, AR paragraph, no manual placement needed); mobile collapses to a single column, same paired order, so it stacks EN-paragraph-then-its-AR-paragraph rather than by language block, same mobile-pairing fix already used on the coming-soon page. **Structure/layout built 2026-08-26**, `dkd-about-row` Row Layout (heading full-width on top, real approved bed image left, self-contained Custom HTML block right holding 3 EN/AR paragraph pairs in a CSS Grid with auto-placement, no manual grid-column/row). Verified both breakpoints live: desktop pairs each row correctly, mobile collapses to a single column stacking EN1/AR1/EN2/AR2/EN3/AR3 in source order, matching the coming-soon page's pairing fix. Left unchecked (`-` not `x`) since the content inside is still bracketed placeholder text, not final, real copy is blocked on Dema, see Phase 2, and this becomes a straight content swap into the same containers once it arrives, no rebuild needed.
