...
- **JSON-LD structured data moved from a Custom HTML content block to a Code Snippets `wp_head` snippet, 2026-08-27.** Originally added directly inside the Contact section's page content (see `TO-DO-List.md`'s "Add the store address/hours to the site's JSON-LD structured data" entry for the original build). Relocated because content-block placement was fragile, any future edit or rebuild of that section could silently delete the structured data along with it, with no obvious signal anything broke. Now lives in `wp-custom-edits/PHP/jsonld-furniturestore.php`, hooked to `wp_head`, **sitewide** (no `is_front_page()` gate, corrected after an initial homepage-only draft was caught as wrong, this is business/entity identity data that belongs on Shop/Product/Cart/Checkout/Thank You/legal pages too, not just the homepage). Formatted (not minified) inside the file to match the coming-soon page's own JSON-LD block in `index.html`, since this repo is headed for a public GitHub portfolio and should read cleanly for a human reviewer, not just a crawler, reformatting was verified data-identical to the original minified JSON via a round-trip parse before the file was rewritten. The old Custom HTML block in the Contact section needs deleting once the new snippet is confirmed live, to avoid duplicate structured data on that page, see `TO-DO-List.md`.
- **Testimonials section, Additional CSS, three rules added 2026-08-27**, full mechanism writeup lives in `docs/handoff/homepage-and-header-footer.md`'s Testimonials section, not duplicated here in full. Short version: the native Kadence Testimonials block has no display-size control for its avatar image anywhere in its own settings panel, and fixing that with a straightforward size cap on the wrapper chain exposed two further layout bugs cascading from it (off-center avatar, then a Kadence Column found to be overflowing its own parent by more than 2x). All three confirmed and fixed via live DOM inspection (`getBoundingClientRect`/`getComputedStyle`), not assumed. Scoped entirely under `.dkd-tsm-section` / `:has(.kt-blocks-carousel)`, can't affect anything outside that section. See `wp-custom-edits/CSS/additional-css.css` for the full three-rule block with inline documentation.
