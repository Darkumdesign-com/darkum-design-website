...
## Demo product page fixes, 2026-09-19 (later session): image cropping, totals labels, layout

**Shop-grid image cropping.** The Bed Frame product photos are all landscape, which suits the product page but got cropped awkwardly in the Shop page's square 4-column grid. Rather than padding every image to a square canvas in Photoshop, Sobhy fixed it site-wide via Customizer > WooCommerce > Product Images, switching to the "Uncropped" option so WooCommerce displays images at their native aspect ratio instead of forcing a 300x300px square crop.

**APF frontend totals labels renamed, with two false leads along the way.** Renamed the plugin's hardcoded "Product total" label to "Base subtotal" via a `gettext` filter, `wp-custom-edits/PHP/apf-totals-labels.php`. Final live labels: "Base subtotal", "Options total" (kept as-is), "Grand total" (kept as-is).
- **Code Snippets gotcha:** pasting the file's own leading `<?php` tag into the Code Snippets plugin's editor caused "Parse Error: unexpected '<' on line 1", the plugin already runs snippets inside its own PHP context. The tag has to be stripped before pasting; the mirror `.php` file keeps it only so it's a valid standalone file in the repo. Documented in the file's own header comment for future snippets.
- **False lead 1, wrong reference version:** the filter was first written checking `$domain !== 'advanced-product-fields-for-woocommerce'`, based on inspecting the free WordPress.org version of Advanced Product Fields. This site runs the paid Lifetime tier bought directly from StudioWombat.com, whose actual text domain doesn't match, so the domain check silently did nothing, confirmed dead even after a GoDaddy cache flush and a private-window test.
- **False lead 2, Polylang priority:** suspected Polylang's own `gettext` filter was overriding the custom one, raised filter priority to `PHP_INT_MAX`. Sobhy disabled Polylang entirely to test, no change, ruling this out directly.
- **Real fix:** dropped the `$domain` check and matched on `$original` text alone (`'Product total'`), works regardless of which domain the paid build actually uses. Confirmed live immediately.
- **Scope note:** this is sitewide, not per-product. The label works while Bed Frame is the only product; once more products exist this would need a dynamic, per-product version instead.

**Product-page cart layout, quantity + Add to Cart + totals alignment.** Two issues fixed in `wp-custom-edits/CSS/additional-css.css` (lines 802-825, mirrored live):
1. The quantity input and Add to Cart button were left-aligned with dead space beside the button instead of the button filling the row. Fixed with a `form.cart { display: flex; ... }` rule (WooCommerce's own standard class, applies to every product), quantity fixed-width and the button set to grow and fill the remaining space.
2. After that fix, the totals block above (Product/Options/Grand total) still didn't line up on the right edge with the button row below it. Sobhy diagnosed this directly via DevTools computed styles and confirmed no competing button-width rule was the cause. The real cause: `.wapf-product-totals .wapf--inner` had no width constraint of its own. Fixed with `.wapf-product-totals .wapf--inner { max-width: 96%; }`, confirmed live.

See `docs/handoff/homepage-and-header-footer.md` for the same-day floating WhatsApp button shadow fix (sitewide element, not configurator-specific).
