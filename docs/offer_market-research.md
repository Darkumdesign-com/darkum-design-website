# Darkum Design, Offer & Market Research (Archived)

**Archived:** 2026-08-16
**Why:** Bido offered a flat rate of **EGP 15,000 for the entire engagement** (logo, social media presence, coming-soon page, full website build, Bed Frame configurator), superseding the item-by-item proposal and the freelancer market research that had been gathered to validate it. This file preserves that work for reference. See `docs/HANDOFF.md` §6 for current status.

---

## 1. Original Proposal / Business Terms
...
|---|---|---|---|
| 1 | Logo & Brand Mark | 5,000 | ✅ Validated against 2 real Khamsat quotes (see §2 below) |
| 2 | Social Media Presence | 3,000 | ⏳ Sobhy's estimate, freelancer quote request in progress (see §3 below) |
...
**Confirmed terms (as of the last item-by-item pricing pass):**
- **Logo**, billable, complete and delivered, price validated.
...
- **Website**, the coming-soon page (already built) plus the future full WordPress/WooCommerce build, including the Bed Frame configurator as its own explicit line item per the existing scoping note.
- **Bed Frame configurator pricing is plugin-agnostic**, same fee regardless of which plugin (APF or ThemeComplete) Bido eventually approves, since the conditional-logic blueprint work is identical either way.
...
- Sobhy's response: declined to argue the number directly, and said he's doing item-by-item market research instead (the ongoing Khamsat quote-gathering, see below) and will share findings once complete.
...
**Noted for the record, not yet verified:** the 10,000 EGP competing quote and the friend's claimed track record are both unconfirmed third-hand claims (no name, no portfolio, no direct contact), treated as a negotiation anchor, not as market data. The market-research approach below was the response at the time: currency- and scope-explicit, sourced directly from working freelancers, and not vulnerable to either tactic above.

**Outcome:** Bido subsequently offered a flat rate of EGP 15,000 for the entire engagement (2026-08-16), which Sobhy is treating as the operative arrangement going forward, see `docs/HANDOFF.md` §6.

---

## 2. Market Research, Freelancer Quotes

### Logo

- **Pricing validated (2026-08-13):** Sobhy contacted 2 Khamsat freelancers for quotes on logo work matching this exact spec (multi-format/multi-color/multi-layout export). Had full conversations with both; both confirmed they'd deliver all 176 files at the real spec, $80 (~4,000 EGP) and $40 (~2,000 EGP). Combined with Sobhy's own instinct (max 5,000 EGP), the proposal's original 12,000 EGP figure (sourced from Egyptian agency-blog marketing pages) was confirmed too high for this context.
...
**USD is the reference currency for all freelancer quotes** (EGP shown only as an approximate, date-stamped conversion), EGP has moved noticeably within 2026 alone (46.6–54.9 range over the past year per market data), so a fixed EGP figure agreed on today can look wrong in a few weeks purely from currency movement, independent of the actual value of the work.

Average ≈ $118 (range $40–$250), median $90, pointing *above* Sobhy's original 3,000 EGP (~$60) proposal estimate (opposite direction from the logo, where real quotes came in below the agency-blog figure). Adding Ahmed H.'s $50 pulls the average down from the 3-quote figure ($140) and puts it closer to Sobhy's own estimate. **None of these four had been through the confirm-the-exact-scope conversation that validated the two logo quotes**, Ahmed H.'s reply is the closest to that bar on its own (itemized against the brief unprompted), but still hadn't had a back-and-forth confirming he'd actually deliver at the full spec.

### Website, Full Build

To continue the same market-research approach used for the Logo and Social Media Presence, a freelancer/developer quote request brief was drafted for the Website, Full WordPress/WooCommerce Build item, **explicitly excluding** the Bed Frame Configurator (priced and sourced separately below).

- English version: `docs/WEBSITE-FREELANCER-REQUEST.md`
- Egyptian Arabic version: `docs/WEBSITE-FREELANCER-REQUEST-AR.md`

Both briefs were rewritten 2026-08-13 to reflect the resolved scope (animation approach, bilingual approach, see `docs/HANDOFF.md` §4) once those questions were settled with Sobhy, so the brief stated the final architecture directly rather than hedging with an assumption. Sobhy to send to 3+ developers, same validation approach as Logo/Social Media.

### Bed Frame Configurator, pricing notes
...
- **Scoping note:** even with the right plugin, wiring up the full conditional chain (position → insertions → size, repeated per position) is several hours of careful setup, this was flagged as its own line item in the original proposal rather than folded into general "website build."

### Coming-Soon Page

**Two offers received on Khamsat (2026-08-13), both show strong, itemized scope confirmation (closest to the logo-quote validation bar, on the first reply, unprompted):**
...
**Both freelancers independently asked the same clarifying question, unprompted, a real gap in the original brief:** does "Notify Me" need to actually capture/store/send the submitted email (real backend), or just show a success/error message (UI + validation only, no data persistence)? The brief hadn't specified this. Checked against the live page at the time (`js/script.js`): the form was explicitly **front-end only**, code comment read "front-end only placeholder until a real backend/service is wired up."

**Resolved (2026-08-13):** if real capture is wanted, the service is **Formspree**, Sobhy's established choice, already used on his own site (SobhyDev), free-tier available, no server-side/backend code needed. This gave both freelancers a concrete, identical integration to price against, "Formspree integration" rather than a vague "real backend."

**Decided and implemented (2026-08-13):** Sobhy chose Formspree-connected (not UI-only). Live and verified, see `docs/HANDOFF.md` §5 for implementation details.
