...
**Open bug carried to next session:** the `#ticker-status` screen-reader announcer ("Slide 2 of 3 / الشريحة 2 من 3") is rendering visibly on the page instead of staying hidden — the `.sr-only` class fix above may not have been applied/saved yet, or needs rechecking. **First task next session**, see §7.
...
- [ ] **Fix `#ticker-status` rendering visibly on the homepage hero ticker** instead of staying screen-reader-only (`role="status" aria-live="polite"` text currently showing on-page) — likely the `.sr-only` CSS class needs to be (re)added to the Custom HTML block's inline `<style>`, see the Homepage build section in §4. **First task, next session.**
