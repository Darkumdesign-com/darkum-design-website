...
**Flag for whoever reads this next:** the paragraph above (real photography, background removed) does not obviously match the same day's later session, described below, where Bido was sending AI-generated (ChatGPT) reference/concept images, explicitly not real photography, in unannounced batches. Both could be true (real photography of the frame itself, separate from AI-generated material/insertion concept art), but this wasn't reconciled during the later session, worth confirming with Sobhy rather than assuming either paragraph is stale.
