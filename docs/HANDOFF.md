...
- **Hosting (under consideration 2026-08-15):** GoDaddy Managed WordPress hosting. Not yet started — see §7 for the plan being discussed (build on GoDaddy's provided temporary domain, no DNS changes until go-live, keeping the current Vercel-hosted coming-soon page fully live and undisturbed throughout the build).
...
- [ ] **Under discussion (2026-08-15):** start the full WordPress build on GoDaddy Managed WordPress hosting. Plan being considered: build on GoDaddy's own temporary domain (provided automatically with Managed WordPress, no DNS changes needed) while `darkumdesign.com` keeps serving the current coming-soon page on Vercel untouched; once the build is complete, tested, and approved, point `darkumdesign.com`'s DNS at GoDaddy to go live, replacing the coming-soon page. This is independent of the registrar-transfer lock above (see §4) — not yet started, awaiting Sobhy's go-ahead.
